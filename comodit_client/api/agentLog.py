...
class AgentLog(JsonWrapper):
    """
    An Agent log. An agent log describes actions received by user
    """
...
class AgentLogCollection(Collection):
    """
    Agent logs collection. This is a read-only collection. Also, only
    C{list} operation is implemented.
    """

    def _new(self, json_data = None):
        return AgentLog(json_data)
